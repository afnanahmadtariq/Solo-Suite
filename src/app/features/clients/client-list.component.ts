        ...
        <table class="min-w-full divide-y divide-theme">
          <thead class="table-header">
            <tr>
              ...
              <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-muted uppercase tracking-wider">Contact</th>
          ...
          <tbody class="divide-y divide-subtle">
            @for (client of clientService.clients(); track client.id) {
              <tr class="table-row transition-colors">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-body">{{ client.email }}</div>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right">
                  <div class="flex gap-1 justify-end">
                    <button (click)="openEditModal(client)" class="p-2 text-muted hover:text-orange-400 active:text-orange-500 hover:bg-orange-500/10 active:bg-orange-500/20 active:scale-95 rounded-lg transition-all" aria-label="Edit client"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button>
                    <button (click)="deleteClient(client.id)" class="p-2 text-muted hover:text-red-400 active:text-red-500 hover:bg-red-500/10 active:bg-red-500/20 active:scale-95 rounded-lg transition-all" aria-label="Delete client"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                  </div>
                </td>
              </tr>
            } @empty {
              <tr><td colspan="4" class="px-6 py-16 text-center text-subtle text-sm">No clients yet. Click "Add Client" to get started.</td></tr>
            }
          </tbody>
        </table>
      </div>
        ...
        <div class="bg-surface rounded-xl p-8 max-w-md w-full border border-theme shadow-2xl modal-panel" style="background:var(--modal-bg)" (click)="$event.stopPropagation()">
          ...
          <form [formGroup]="form" (ngSubmit)="saveClient()" class="space-y-5">
            <div>
              ...
              @if (form.get('name')?.touched && form.get('name')?.errors?.['required']) { <p class="field-error">Name is required</p> }
              @if (form.get('name')?.touched && form.get('name')?.errors?.['minlength']) { <p class="field-error">Name must be at least 2 characters</p> }
            </div>
            <div>
            ...
            <div>
              ...
              @if (form.get('email')?.touched && form.get('email')?.errors?.['required']) { <p class="field-error">Email is required</p> }
              @if (form.get('email')?.touched && form.get('email')?.errors?.['email']) { <p class="field-error">Must be a valid email address</p> }
            </div>
            <div>
              ...
              @if (form.get('phone')?.touched && form.get('phone')?.errors?.['required']) { <p class="field-error">Phone is required</p> }
              @if (form.get('phone')?.touched && form.get('phone')?.errors?.['pattern']) { <p class="field-error">Enter a valid phone number</p> }
            </div>
            <div>
            ...
            <div class="flex gap-3 pt-2">
              ...
              <button type="submit" [disabled]="form.invalid" class="btn-primary flex-1">{{ editingClientId() ? 'Save Changes' : 'Add Client' }}</button>
            </div>
          </form>
        </div>
      </div>
...
export class ClientListComponent {
  ...
  openAddModal() { this.editingClientId.set(null); this.form.reset({ name: '', company: '', email: '', phone: '', status: 'Active' }); this.showModal.set(true); }
  openEditModal(client: Client) { this.editingClientId.set(client.id); this.form.patchValue(client); this.showModal.set(true); }
  closeModal() { this.showModal.set(false); }
  saveClient() {
    if (this.form.invalid) { this.form.markAllAsTouched(); return; }
    const v = this.form.getRawValue(); const id = this.editingClientId();
    id ? this.clientService.updateClient(id, v) : this.clientService.addClient(v);
    this.closeModal();
  }
  async deleteClient(id: number) {
    if (await this.popupService.confirm({ title: 'Delete Client', message: 'Are you sure? This will also remove related projects and invoices.', confirmText: 'Delete', cancelText: 'Cancel', type: 'danger' }))
